...
function createAsteroid() {
  if (Math.random() > 0.7) {
    ...
    const x = -50;
    const y = Math.random() * 100;
    const size = Math.random() * 15 + 10; // kecil
    ...
    // Tambahkan pseudo-tonjolan kasar kecil
...
function initSpaceEffects() {
  ...
  setInterval(createMeteor, 700);
  setInterval(createAsteroid, 1200);
  setInterval(createGalaxy, 8000);
}
...
// ================== SKILL SYSTEM (FIXED, SYNC, LEFT → RIGHT) ==================
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // ========== LOADING ANIMATION FOR PORTFOLIO (FIXED) ==========
  ...
  // Project URLs untuk portfolio yang sudah diperbarui
  const projectURLs = {
    "moodai-tracker": "#", // Belum ada link
    "nulief-property":
  ...
  function startLoadingAnimation(url) {
    ...
    loadingInterval = setInterval(() => {
      // Increment progress
      currentProgress += Math.random() * 3 + 1; // Random antara 1-4%
      ...
      if (currentProgress >= 100) {
        ...
        // Delay untuk menampilkan pesan completed sebelum redirect
        setTimeout(() => {
          if (targetUrl && targetUrl !== "#") {
            window.open(targetUrl, "_blank");
          }
          hideLoading();
        }, 800); // Tunggu 800ms agar user melihat "complete"
      }
  ...
  // View button click handler - PERBAIKAN UTAMA
  viewBtns.forEach((btn) => {
    btn.addEventListener("click", function (e) {
      // Cek apakah ini link <a> atau button
      if (this.tagName === "A") {
        // Untuk link, cegah default behavior dan jalankan loading animation
        e.preventDefault();
        const url = this.getAttribute("href");
        startLoadingAnimation(url);
      } else {
        // Untuk button tanpa link
  ...
  // Fungsi untuk membuat bintang di loading screen
...
// ========== MOBILE MENU FUNCTIONALITY - BARU ==========
document.addEventListener("DOMContentLoaded", function () {
  const mobileMenuBtn = document.getElementById("mobileMenuBtn");
  const mobileMenuOverlay = document.querySelector(".mobile-menu-overlay");
  const navOverlay = document.getElementById("navOverlay"); // Jika ada overlay hitam
  ...
  // Fungsi untuk menutup menu dan reset semua sub-menu
  function closeMobileMenu() {
    mobileMenuOverlay.classList.remove("active");
    if (navOverlay) navOverlay.classList.remove("active");
    mobileMenuBtn.innerHTML = '<i class="fas fa-bars"></i>';
    
    // Reset semua sub-menu: hapus class 'active' dari semua dropdown
    mobileNavDropdowns.forEach((dropdown) => dropdown.classList.remove("active"));
  }

  // Toggle mobile menu overlay dengan reset sub-menu saat menutup
  mobileMenuBtn.addEventListener("click", function (e) {
    e.stopPropagation();
    if (mobileMenuOverlay.classList.contains("active")) {
      // Jika menu sedang aktif, tutup dan reset
      closeMobileMenu();
    } else {
      // Jika menu tidak aktif, buka
      openMobileMenu();
    }
    console.log("Overlay toggled:", mobileMenuOverlay.classList.contains("active"));
  });

  // Toggle sub menu untuk Tentang
  ...
  // Tutup menu saat klik di luar (gunakan closeMobileMenu untuk konsistensi)
  ...
  // Tutup menu saat klik link (gunakan closeMobileMenu)
  ...
  // Tutup sub menu saat klik link di dalam sub menu (gunakan closeMobileMenu)
  ...
  // Opsional: Tutup menu jika klik di dalam overlay tapi bukan di sub-menu (untuk safety)
